...
class PembubaranKoperasiController extends Controller
{
  const PATH_IMAGE = "pembubaran_koperasi/image/";
  function __construct()
  {
    ...
    view()->composer('content.pembubaran_koperasi.*', function ($view) {
      $view->with('page_title', ucwords('Pembubaran Koperasi'));
    });
  }
  ...
  public function store(Request $request)
  {
    ...
    $update = PembubaranKoperasi::find($balik_nama_waris_id);
    $update->update([
      'checklist' => $checklist ? 1 : null,
      'tgl_checklist' => $request->tgl_checklist,
      'rekomendasi' => strtolower($request->rekomendasi),
      'catatan' => $catatan,
    ]);
  ...
  {
    $title = ucwords("Pendirian Koperasi");
    $procedures = PembubaranKoperasi::with('work_order_assignment')->where('work_order_assignment_id', $work_order_assignment_id)->get();
    ...
    // dd($wo_attachment);
    return view('content.pembubaran_koperasi.form', compact(
      'title',
      'procedures',
  ...
  public function detail($work_order_assignment_id)
  {
    ...
    return view('content.pembubaran_koperasi.detail', compact(
      'title',
      'procedures',
      'work_order_assignment_id',
      'wo_attachment',
